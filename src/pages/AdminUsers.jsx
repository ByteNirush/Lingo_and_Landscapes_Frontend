          ...
          <div className="overflow-x-auto">
              ...
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50">
                  ...
                  <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Passport</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {users.map((user) => (
                  <tr key={user.id} className="group transition hover:bg-slate-50/80">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-bold text-white ${user.role === 'admin' ? 'bg-linear-to-br from-purple-500 to-indigo-600' : 'bg-linear-to-br from-crimson-400 to-crimson-600'}`}>
                    ...
                    <td className="px-5 py-4">
                      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${user.role === 'admin' ? 'bg-purple-100 text-purple-800' : 'bg-emerald-100 text-emerald-800'}`}>
                        {user.role}
                      </span>
                    </td>
                    <td className="px-5 py-4 text-slate-500">{user.country || '-'}</td>
                    <td className="px-5 py-4 text-slate-500">{user.passport_number || '-'}</td>
