  ...
  const learningPace =
    bookings.length === 0
      ...
      : bookings.length < 3
        ? "Building momentum"
        : "Strong consistency";
  const nextSessionLabel = nextBooking?.slot?.date
    ? formatLongDate(nextBooking.slot.date)
    : "No upcoming session";
    ...
    <div className="fade-in shell py-12 md:py-16">
        ...
        <div className="relative grid gap-8 lg:grid-cols-[1.15fr_0.85fr] lg:items-end">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-sm leading-7 text-slate-200 sm:text-base">
              Track your sessions, check your next booking, and jump straight
              back into learning.
            </p>
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              <Link
                to="/slots"
                className="btn-primary bg-white text-slate-950 hover:bg-slate-100 hover:from-white hover:to-slate-100"
              >
                Browse New Slots
              </Link>
              <Link
            ...
            {loading ? (
              ...
              <>
                ...
                <div className="mt-3 text-sm text-slate-200">
                  {nextBooking.slot?.time}
                </div>
                <div className="mt-3 text-sm leading-relaxed text-slate-300">
                  {nextBooking.slot?.description ||
                    "A private live session is ready for you."}
                </div>
              </>
            ) : (
              <>
                ...
                <div className="mt-3 text-sm leading-relaxed text-slate-300">
                  Book a slot to lock in your next Nepali lesson and get into a
                  steady rhythm.
                </div>
              </>
            )}
      ...
      <section className="mt-8 grid gap-6 md:grid-cols-3">
        <StatCard
          ...
          detail="Total confirmed demo sessions across your account."
          tone="bg-linear-to-r from-crimson-500 to-crimson-600"
        />
        <StatCard
          label="Pending Requests"
          value={loading ? "—" : pendingRequests.length}
          detail="Demo requests waiting for tutor review."
            ...
            {loading ? (
              ...
              <div className="rounded-2xl border border-dashed border-slate-200 bg-white px-5 py-10 text-center">
                ...
                <p className="mt-3 text-sm text-slate-500">
                  No bookings yet. Reserve a slot to start your dashboard
                  history.
                </p>
              </div>
            )}
        ...
        <div className="space-y-6">
          <div className="card overflow-hidden">
            ...
            <p className="mt-3 text-sm leading-relaxed text-slate-500">
              Keep your request flow active so the tutor can schedule the next
              session as soon as they are ready.
            </p>
            ...
            <div className="space-y-4 text-sm text-slate-600">
              <div className="flex items-center justify-between gap-3 border-b border-slate-100 pb-3">
                ...
                <span className="font-semibold text-nepal-dark">
                  {user?.full_name || user?.name || "Learner"}
                </span>
              </div>
              <div className="flex items-center justify-between gap-3 border-b border-slate-100 pb-3">
                <span>Email</span>
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="font-semibold text-nepal-dark">
                  Demo requests and confirmed sessions
                </span>
              </div>
            </div>
          </div>
