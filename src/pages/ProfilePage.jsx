...
const ProfileField = ({ label, value, icon }) => (
  <div className="flex items-start gap-4 rounded-xl border border-slate-200/60 bg-white/60 p-4">
    ...
    <div className="min-w-0 flex-1">
      ...
      <div className="mt-1 truncate text-sm font-semibold text-nepal-dark">{value || '—'}</div>
    </div>
  </div>
);
    ...
    <div className="fade-in shell py-12 md:py-16">
      ...
      <section className="mt-8 grid gap-6 lg:grid-cols-[1fr_0.5fr]">
        <div className="card-soft">
          ...
          <h2 className="text-2xl font-display font-bold text-nepal-dark">Account Details</h2>

          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            <ProfileField label="Full Name" value={user?.full_name} icon="👤" />
            <ProfileField label="Email Address" value={user?.email} icon="📧" />
            <ProfileField label="Country" value={user?.country} icon="🌍" />
            <ProfileField label="Passport Number" value={user?.passport_number} icon="🛂" />
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="card overflow-hidden">
            ...
            <h3 className="text-2xl font-display font-bold text-nepal-dark">Your Journey</h3>
            <p className="mt-3 text-sm leading-relaxed text-slate-500">
              Your profile is complete and ready for booking sessions. Keep your details up to date for a seamless experience.
            </p>
          </div>
        </div>
      </section>
