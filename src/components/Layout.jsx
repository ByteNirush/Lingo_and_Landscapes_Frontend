...
export default function Layout() {
  ...
  const publicNavLink = (sectionId, label) => {
    const isActive = location.pathname === '/' && location.hash === `#${sectionId}`;
      ...
      <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/80 backdrop-blur-md">
        <div className="shell flex h-16 items-center justify-between gap-3">
          <Link to="/" className="group flex items-center gap-2.5">
            ...
            <div>
              <div className="font-display text-lg font-bold leading-none text-nepal-dark">नमस्ते</div>
              <div className="mt-0.5 text-[10px] font-semibold uppercase leading-none tracking-widest text-crimson-500">Learn Nepali</div>
            </div>
          </Link>
          ...
          >
            <div className="space-y-1.5">
              ...
              <span className={`block h-0.5 w-6 bg-nepal-dark transition-opacity duration-200 ${menuOpen ? 'opacity-0' : ''}`} />
              <span className={`block h-0.5 w-6 bg-nepal-dark transition-transform duration-200 ${menuOpen ? '-translate-y-2 -rotate-45' : ''}`} />
            </div>
          </button>
        </div>
          ...
          <div id="mobile-menu" className="shell space-y-3 border-t border-slate-200 py-4 md:hidden">
            ...
            {isAuthenticated ? (
              ...
              <div className="flex flex-col gap-2 pt-2">
                <Link to="/login" onClick={() => setMenuOpen(false)} className="btn-ghost justify-center border border-slate-300 text-sm">Login</Link>
                <Link to="/signup" onClick={() => setMenuOpen(false)} className="btn-primary justify-center text-sm">Get Started</Link>
              </div>
            )}
          </div>
        )}
